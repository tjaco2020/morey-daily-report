...
export function LocationsClient({ initial }: { initial: Row[] }) {
  ...
  async function add(e: React.FormEvent) {
    ...
    try {
      ...
      setRows(
        [...rows, data].sort((a, b) => a.display_order - b.display_order),
      );
      setName("");
      setOrder("");
      showMsg("Added.", true);
  ...
  async function remove(row: Row) {
    if (
      !window.confirm(
        `Delete location "${row.name}"? Any terminals currently assigned to it will be unassigned.`,
      )
    )
      return;
    setBusy(true);
      ...
      <section className="bg-white rounded-bubble shadow-card border border-slate-100/80 p-5">
        ...
        <form onSubmit={add} className="grid grid-cols-1 sm:grid-cols-4 gap-2">
          <input
            ...
            placeholder="Name (e.g. Kong Store)"
            required
          ...
          <input
            ...
            placeholder={`Order (default: ${nextOrder})`}
